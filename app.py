...
st.caption("Strict Elliott + Structural Swing + Fibonacci + Trend Filters + ML Buy/Sell Signals")
...
def build_ml_features(df: pd.DataFrame, timeframe: str, ticker: str) -> pd.DataFrame:
    ...
    # There might still be NaNs in SMA distances if very early
    feat = feat.dropna()
...
def train_ml_model(df_all: pd.DataFrame, feature_cols: list):
    ...
    # Basic split (we don't really need metrics here, but OK)
...
def predict_for_latest(df: pd.DataFrame, clf, feature_cols: list):
    """
    ...
    # build single-row features without label
    last = df.iloc[-1:]
    ...
    # past returns
    row["Ret_3"] = last["Close"].pct_change(3).iloc[-1]
    row["Ret_5"] = last["Close"].pct_change(5).iloc[-1]
    row["Ret_10"] = last["Close"].pct_change(10).iloc[-1]
        ...
        if not all_daily and not all_weekly and not all_hourly:
            ...
            if all_daily:
                ...
                for tk in default_tickers:
                    ...
                    proba = predict_for_latest(df_d_curr, daily_model, feature_cols)
                    if proba is None:
                        continue
            ...
            if all_weekly:
                ...
                for tk in default_tickers:
                    ...
                    proba_w = predict_for_latest(df_w_curr, weekly_model, feature_cols)
                    if proba_w is None:
                        continue
                    rows_w.append({
                        "Ticker": tk,
